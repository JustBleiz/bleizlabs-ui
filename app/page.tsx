...
const components: ComponentLink[] = [
  ...
  },
  {
    ...
    name: 'PieChart',
    description: 'SVG pie + donut chart for categorical composition (full 360°). data: {name, value, color?}[]. variant: pie | donut (donut innerR = 0.6× outer). Optional centerLabel slot (donut hole), optional showLabels with auto-hide for slices <10%. Full interactive a11y per segment — roving tabindex, Arrow/Home/End/Space/Enter/Escape, sr-only Name/Value/% table. Complements UsageDonut (partial progress) — different semantics.',
    phase: 'Phase 6 — Specialized',
    status: 'available',
  },
  {
  ...
  },
  {
    ...
    description: 'Two-date range picker with one Calendar popover. Hover preview, range validation (from ≤ to, min/max/disabledDates), keyboard nav, locale-aware via Intl.DateTimeFormat. Optional two-month side-by-side variant.',
    phase: 'Phase 10 — Complex Interactive',
    status: 'available',
  },
  {
    ...
    name: 'TimeInput',
    description: 'Inline HH:MM(:SS) spinbutton trio for time entry. 12h/24h hourCycle variants, optional seconds, AM/PM toggle, keyboard nav (Tab between fields, Arrow Up/Down to increment). Bespoke role="spinbutton" per APG.',
    phase: 'Phase 10 — Complex Interactive',
    status: 'available',
  },
  {
    ...
    name: 'TimePicker',
    description: 'Popover combobox with scrollable hour/minute listbox columns (macOS-style). Step filter, format consistency with TimeInput. Reuses Popover + floating logic from existing components.',
    phase: 'Phase 10 — Complex Interactive',
    status: 'available',
  },
  {
    ...
    name: 'DateTimePicker',
    description: 'Combined date + time picker — Calendar + TimeInput compound in a single popover. Output type: Date with time component. Min/max constraints span both date and time.',
    phase: 'Phase 10 — Complex Interactive',
    status: 'available',
  },
  {
