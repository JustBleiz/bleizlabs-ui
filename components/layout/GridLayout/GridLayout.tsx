...
/**
 * GridLayout — multi-column CSS Grid layout atom (Phase 1 L5, v0.5.5).
 *
 ...
 * (sidebar + main + aside).
 *
 * Promoted from scout-hub `2026-04_e05-gan-reattack` B4 `/settings/icp`
 * metaGrid 2x2 density pattern (memory `project_b4_settings_icp_plateau`).
 * Replaces the rejected "DataRow grid variant" — DataRow is a single
 * label/value pair, while GridLayout owns arbitrary content cells.
 *
 * @layer   atom (layout)
 ...
 *          `GridLayout.module.scss` (SCSS-side dep not surfaced in @deps
 ...
 *          `columns` prop accepts a number (`repeat(N, minmax(0, 1fr))`
 *          shorthand for equal columns with overflow protection) OR a raw
 *          CSS grid template string for arbitrary tracks. The `responsive`
 *          prop cascades mobile-first: omitted breakpoints inherit from
 *          the next-smaller (or the base `columns`) via SCSS `var()`
 *          fallback chain. The `forwardRef` ref type is fixed to
 *          `HTMLDivElement`; with `asChild` the actual DOM node may
 *          differ — accepted trade-off of the asChild pattern (no
 *          polymorphic generics, mirrors Stack/Inline precedent).
 *
 * @example
 * // Equal 3-column grid
