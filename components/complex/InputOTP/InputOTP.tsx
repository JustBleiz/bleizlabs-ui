  ...
  function InputOTP(
    ...
    const focusInput = useCallback((caretIndex?: number) => {
      ...
      if (caretIndex !== undefined) {
        const clamped = Math.max(
          0,
          Math.min(caretIndex, latestValueRef.current.length),
        );
    ...
    const handleChange = useCallback(
      (event: ChangeEvent<HTMLInputElement>) => {
        ...
        const truncated =
          filtered.length > maxLength ? filtered.slice(0, maxLength) : filtered;
        if (truncated !== raw) {
...
export const InputOTPGroup = forwardRef<HTMLDivElement, InputOTPGroupProps>(
  function InputOTPGroup({ className, children, ...rest }, ref) {
    return (
