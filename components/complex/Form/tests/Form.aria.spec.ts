...
test.describe('Form — ARIA semantics', () => {
  ...
  test('Form with aria-label exposes accessible form landmark', async ({
    page,
  }) => {
  ...
  test('Form.Submit asChild renders a button of type="submit"', async ({
    page,
  }) => {
  ...
  test('Form.Submit native fallback renders <button type="submit">', async ({
    page,
  }) => {
  ...
  test('Form noValidate variant sets noValidate attribute on <form>', async ({
    page,
  }) => {
    // Section 2 uses noValidate.
    const form = page.getByRole('form', { name: /custom validated/i });
    const noValidate = await form.evaluate(
      (el) => (el as HTMLFormElement).noValidate,
    );
  ...
  test('Required input has correct ARIA + native required attribute', async ({
    page,
  }) => {
