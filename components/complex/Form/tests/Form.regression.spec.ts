...
test.describe('Form — regression cases (Radix issues + CV API edges)', () => {
  ...
  test('FM-R02: filled required input allows submit', async ({ page }) => {
    ...
    await page
      ...
      .first()
      .click();
    await expect(page.getByText(/submitted: valid@example\.com/i)).toBeVisible();
  });
  ...
  test('FM-R05: noValidate suppresses browser popup AND fires onSubmit', async ({ page }) => {
    ...
    await expect(page.getByText(/fail: username must be ≥3 chars/i)).toBeVisible();
  });

  test('FM-R06: noValidate + valid input fires onSubmit with success', async ({ page }) => {
  ...
  test('FM-R07: multi-field form blocks on first invalid field', async ({ page }) => {
  ...
  test.skip('FM-R08: multi-field form submits when all required filled', async ({ page }) => {
  ...
  test.skip('FM-R09: pattern mismatch on phone input blocks submit', async ({ page }) => {
    await page.getByRole('textbox', { name: /full name/i }).fill('Alice');
    await page
      .getByRole('textbox', { name: /^email$/i })
      .nth(2)
    ...
    await expect(page.getByText(/^submitted: \{.*"name":"Alice".*\}/i)).not.toBeVisible();
  });
  ...
  test('FM-R10: Form.Submit asChild merges type="submit" onto lib Button', async ({ page }) => {
    const sendBtn = page.getByRole('button', { name: /^send$/i }).first();
    await expect(sendBtn).toHaveAttribute('type', 'submit');
  });

  test('FM-R11: Form.Submit native fallback renders <button type="submit">', async ({ page }) => {
  ...
  test('FM-R12: Form with aria-label exposes accessible form role', async ({ page }) => {
    const form = page.getByRole('form', { name: /contact form/i });
    await expect(form).toBeVisible();
  });

  test('FM-R13: noValidate state mirrors prop on <form> element', async ({ page }) => {
  ...
  test('FM-R18: useFormContext throws when called outside <Form>', async ({ page }) => {
  ...
  test('FM-R19: Form fires onSubmit only ONCE per submit click', async ({ page }) => {
    ...
    const matches = await page.locator(`text=/submitted: once@example\\.com/i`).count();
    expect(matches).toBe(1);
  });

  test('FM-R20: empty form with no required fields submits cleanly', async ({ page }) => {
  ...
  test('FM-R21: Form hydrates without console errors', async ({ page }) => {
    ...
    await page.goto(URL);
    await page.waitForLoadState('networkidle');
    expect(errors.filter((e) => /hydrat|<form>|FormContext/i.test(e))).toHaveLength(0);
  });

  test('FM-R22: Form noValidate prop initial state hydrates correctly', async ({ page }) => {
  ...
  test('FM-R23: <Field> inside <Form> does not trigger infinite re-register loop', async ({
    ...
    expect(errors.filter((e) => /Maximum update depth exceeded/i.test(e))).toHaveLength(0);
  });
});
