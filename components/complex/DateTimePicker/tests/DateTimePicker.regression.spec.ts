 ...
 * - DT-R08 typed "2026-06-01 14:30" (space separator) parses (normalized to T)
 */
...
test.describe('DateTimePicker — regression cases', () => {
  ...
  test('DT-R02 — controlled value 14:30 PM displays correctly', async ({ page }) => {
    ...
    // 12h mode demo defaultValue=2026-05-15T14:30 → display "2026-05-15T14:30"
    // (input string is always ISO-format; 12h only affects TimeInput inside popover)
    await expect(input).toHaveValue('2026-05-15T14:30');
  });
  ...
  test('DT-R05 — year boundary 2026-12-31T23:59 parses', async ({ page }) => {
    ...
    await input.press('Enter');
    await expect(input).toHaveValue('2026-12-31T23:59');
  });
  ...
  test('DT-R06 — withSeconds includes seconds in hidden ISO', async ({ page }) => {
    ...
    const input = inputOf(picker);
    await expect(input).toHaveValue('2026-05-15T09:00:45');
  });

  test('DT-R08 — typed space separator normalizes to ISO T', async ({ page }) => {
    const picker = pickerBy(page, 'Start datetime');
    const input = inputOf(picker);
    await input.focus();
    await input.fill('2026-06-01 14:30');
    await input.press('Enter');
    // Normalized output uses T separator
