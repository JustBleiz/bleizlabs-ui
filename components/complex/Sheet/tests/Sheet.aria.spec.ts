 ...
 * - axe-core zero violations when sheet is OPEN
 */
...
test.describe('Sheet — ARIA compliance (APG dialog-modal)', () => {
  ...
  test('aria snapshot exposes dialog role with accessible name', async ({ page }) => {
    ...
    const sheet = page.getByRole('dialog');
    const snapshot = await sheet.ariaSnapshot();
    expect(snapshot).toContain('dialog');
    expect(snapshot).toContain('Product details');
  });

  test('axe-core zero violations when sheet is open', async ({ page }) => {
    await page.getByRole('button', { name: /open right sheet$/i }).click();
    await page.waitForTimeout(50);
