/**
 * AlertDialog regression spec — Radix closed issues mapped to test cases (E16).
 *
 * Categories:
 *   1. Inherited from Dialog (portal + focus trap + scroll lock + inert)
 *   2. AlertDialog-specific (role, required aria-describedby, overlay blocking,
 *      least-destructive focus, destructive styling, size variants, etc.)
 *
 * Tests marked `test.skip` reference scenarios NOT in the base playground
 * (nested select / toast / form / shadow-DOM / nested alert). First consumer
 * with those use cases should extend the playground and un-skip.
 *
 * Playground: `/components/alert-dialog`
 */
...
test.describe('AlertDialog — Radix regression cases', () => {
  ...
  test.skip('#2690 inherited — clicking toast inside does not close alert', async () => {
    // PLAYGROUND-DEP: needs nested Toast atom (Phase 10 CI14).
  });

  test.skip('#1951 inherited — Escape in nested Select closes Select first, not AlertDialog', async () => {
    // PLAYGROUND-DEP: needs nested Select atom (Phase 10 CI11).
  });
  ...
  test('#2961 inherited — reopening alert after close works', async ({ page }) => {
  ...
  test.skip('#1249 inherited — nested AlertDialog Escape closes innermost', async () => {
    // PLAYGROUND-DEP: needs nested alert scenario.
  });
  ...
  test.skip('#2544 inherited — focus trap always on for modal alerts', async () => {
    // Documented limitation: trap cannot be disabled for AlertDialog (APG requires it).
  });
  ...
  test('#2270 inherited — focus returns to trigger that opened alert', async ({ page }) => {
  ...
  test('#3579 inherited — description id referenced correctly', async ({ page }) => {
    await page.getByRole('button', { name: /open basic alert/i }).click();
    const alert = page.getByRole('alertdialog');
    const describedBy = await alert.getAttribute('aria-describedby');
  ...
  test.skip('#2275 inherited — nested Select arrow keys do not escape alert', async () => {
    // PLAYGROUND-DEP: needs nested Select (Phase 10 CI11).
  });
  ...
  test('#2532 inherited — animation race condition handled', async ({ page }) => {
    ...
    await trigger.click();
    await expect(page.getByRole('alertdialog')).toBeVisible();
    // Rapid close+reopen — must remain open or cleanly reopen without error.
    await page.keyboard.press('Escape');
    await trigger.click();
    await expect(page.getByRole('alertdialog')).toBeVisible();
  });
  ...
  test.skip('#1546 inherited — conditional initialFocusRef works', async () => {
    // PLAYGROUND-DEP: needs custom-focus scenario with ref-accessible target.
  });
  ...
  test('AD-4: closeOnOverlayClick defaults false (Dialog defaults true)', async ({ page }) => {
    await page.getByRole('button', { name: /open basic alert/i }).click();
    // Click near top-left of overlay (safely outside content card)
  ...
  test.skip('AD-9: explicit confirmVariant overrides severity default', async () => {
    // PLAYGROUND-DEP: needs custom-variant scenario.
  });
  ...
  test('AD-11: Custom cancelLabel overrides default', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /stay here/i })).toBeVisible();
  });
  ...
  test.skip('AD-13: onConfirm does not auto-close dialog (consumer controls)', async () => {
    // PLAYGROUND-DEP: needs async-confirm scenario.
  });
