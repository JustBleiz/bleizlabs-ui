...
function shouldRenderScrollbar(
  ...
  const hasScroll =
    orientation === 'vertical' ? ctx.hasVerticalScroll : ctx.hasHorizontalScroll;
...
function useScrollbarContext(): ScrollbarContextValue {
  ...
  if (!ctx) {
    throw new Error(
      '<ScrollAreaThumb> must be rendered inside <ScrollAreaScrollbar>',
    );
  }
  return ctx;
}
...
>(function ScrollAreaScrollbar(
  ...
  const mergedRef = useMemo(
    () => mergeRefs<HTMLDivElement>(ref, setTrackEl),
    [ref, setTrackEl],
  );
  ...
  const handleTrackPointerDown = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>) => {
      ...
      if (orientation === 'vertical') {
        ...
        const thumbY =
          (ctx.metrics.scrollTop / Math.max(1, ctx.metrics.scrollHeight)) *
          trackRect.height;
        ...
        const thumbX =
          (ctx.metrics.scrollLeft / Math.max(1, ctx.metrics.scrollWidth)) *
          trackRect.width;
...
export const ScrollAreaThumb = forwardRef<HTMLDivElement, ScrollAreaThumbProps>(
  function ScrollAreaThumb({ className, style, ...rest }, ref) {
    ...
    const thumbStyle = useMemo<CSSProperties>(() => {
      if (orientation === 'vertical') {
        const visibleRatio =
          ctx.metrics.clientHeight / Math.max(1, ctx.metrics.scrollHeight);
        ...
        const thumbTop =
          (ctx.metrics.scrollTop / scrollableDistance) * trackScrollable;
        return {
        ...
        const visibleRatio =
          ctx.metrics.clientWidth / Math.max(1, ctx.metrics.scrollWidth);
        ...
        const thumbLeft =
          (ctx.metrics.scrollLeft / scrollableDistance) * trackScrollable;
        return {
          width: `${thumbWidth}px`,
    ...
    const handlePointerMove = useCallback(
      (event: ReactPointerEvent<HTMLDivElement>) => {
        ...
        if (orientation === 'vertical') {
          ...
          const visibleRatio =
            metrics.clientHeight / Math.max(1, metrics.scrollHeight);
          const thumbHeight = Math.max(24, trackHeight * visibleRatio);
          const trackScrollable = Math.max(1, trackHeight - thumbHeight);
          const scrollableDistance = Math.max(
            1,
          ...
          const newScrollTop = Math.max(
            ...
            Math.min(
              scrollableDistance,
              dragStartScrollRef.current + ratio * scrollableDistance,
            ),
          );
          ...
          const visibleRatio =
            metrics.clientWidth / Math.max(1, metrics.scrollWidth);
          const thumbWidth = Math.max(24, trackWidth * visibleRatio);
          const trackScrollable = Math.max(1, trackWidth - thumbWidth);
          const scrollableDistance = Math.max(
            1,
