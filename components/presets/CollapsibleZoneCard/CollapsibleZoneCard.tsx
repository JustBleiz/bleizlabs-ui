...
/**
 * CollapsibleZoneCard — universal collapsible info-card preset (CP10, client)
 *
 * @layer   preset (Phase 8 CP10, Tier S promoted v0.8.0; sister to ZoneCard CP9)
 * @apg     https://www.w3.org/WAI/ARIA/apg/patterns/disclosure/
 * @tokens  Inherits 100% of ZoneCard CP9 token surface (R22 — zero new tokens):
 *            density → Card padding/body-gap maps (compact 4/2, comfortable 5/3)
 *            tone   → --color-{success,warning,error,brand,text-secondary}
 *          Adds animation on existing motion tokens:
 *            chevron rotate → --motion-duration-fast + --motion-easing-standard
 *            body grid-rows transition → --motion-duration-normal
 *          NO new seeds required (verified in Phase 1 spec audit vs v0.7.5).
 * @deps    ZoneCard parent contract (type re-exports), Card, Heading, Text,
 *          Inline, Stack, Badge (summaryChip default rendering), cn. Client
 *          component — owns useState for uncontrolled mode + useEffect for
 *          controlled-mode sync. Server-Component INCOMPATIBLE (use ZoneCard
 *          for static cards).
 ...
 * @tested  Playwright (42/42 PASS, dev server :3000):
 *            - keyboard.spec — 6 tests (Space/Enter toggle, Tab/Shift+Tab,
 *              form non-submit, rapid Space)
 *            - focus.spec   — 7 tests (no auto-focus, stays-on-trigger,
 *              focus-visible box-shadow, body focusables via Tab)
 *            - aria.spec    — 9 tests (aria-expanded, aria-controls, body
 *              inert+aria-hidden when closed, single named landmark,
 *              axe-core 0 violations on `<main>`)
 *            - regression.spec — 20 cases (9 from Radix Accordion+Collapsible
 *              closed issues + 11 synthesized from APG)
 *          Smoke (`tests/smoke.spec.ts`): /components/collapsible-zone-card
 *          route added — axe WCAG 2.1 A+AA scan PASS.
 *          Visual screenshots (LOG/component-build/CollapsibleZoneCard/):
 *          desktop-{light,dark}, mobile-{light,dark}, desktop-dark-expanded.
 *          Manual NVDA sweep: PENDING user-driven verification (Windows-only,
 *          requires NVDA running). When performed, append outcome here:
 *          announces "expanded" / "collapsed" state on toggle; summary chip
 *          read order verified before chevron.
 * @regressions See CollapsibleZoneCard.regression.spec.ts for the 20-case
 *          mapping (issue → bug → fix → test derivation).
 *
 * @example
 ...
 *          FinancialBreakdown (2 sites — Income/Expenses zones with summary
 *          chips), ProjectsFinancialOverview (2 sites — Forecast/Actuals
 *          mirror pattern). Universality 3-of-3 PASS (panel_v2 + BleizOS
 *          admin client detail panes + scout-hub admin lead detail sections
 *          all use this pattern). Sister to ZoneCard CP9 — extend with
 *          collapse machinery; `forceMount` opt-out for form-state-bearing
 *          panels (per Radix #2446 + #3601 closed-issue precedent).
 */
