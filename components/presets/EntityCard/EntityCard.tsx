...
/**
 * EntityCard — universal entity grid item preset.
 *
 * Server-Component safe (consumer-supplied click target via `href` renders
 * raw `<a>` — for Next.js routing wrap externally via
 * `<Link>{<EntityCard ...>}</Link>`).
 *
 ...
 *          override. Status badges use Badge color-as-decoration +
 *          label-as-name semantics (color is not the sole carrier of
 *          meaning). Icon slot wrapped with `aria-hidden="true"` — entire
 ...
 *          when href present).
 *
 *          STRUCTURED PROPS by design. Mirrors EntityHero metaStrip API
 *          conventions for preset family consistency. Trades raw flexibility
 *          for: cleaner API surface (≤11 axes), discoverable props via JSDoc,
 *          enforced layout chrome (visual identity consistent across consumers).
 *
 *          CLICK TARGET: two routing patterns supported (use ONE, not both):
 *
 *          (a) Direct `href` (full-page navigation OR non-Next.js apps):
 *          When `href` is provided, EntityCard renders Card with `asChild`
 ...
 *          externally with `<Link href="...">` from `next/link`. Next.js
 ...
 *          (no accent). Set `'left'` for left bar, `'top'` for top bar.
 *          `accentColor` overrides the default brand color (consumer passes
 *          status-driven color, e.g. `'var(--color-success)'` for active
 *          entities).
 *
 ...
 *          grids.
 *
 *          BODY SLOT: optional `body` prop accepts ReactNode for additional
 *          content (metrics rows, charts, custom layouts). Renders below
 *          status badges + meta strip. Consumer composes freely.
 *
 * @example
 * // Compact grid item
 * <EntityCard
 ...
 *   description="24/7 order handling"
 *   badges={[{ label: 'Active', color: 'success' }]}
 *   metaItems={[
 *     { icon: <IconUser size={14} />, label: 'Owner', value: 'Anna K.' },
 *     { icon: <IconClock size={14} />, label: 'Updated', value: '2h ago' },
 *   ]}
 *   accentPosition="left"
 ...
 *
 * @example
 * // Comfortable hero variant
 * <EntityCard
 *   density="comfortable"
 *   title="Shopify migration"
 *   href="/projects/shopify"
 *   description="Full migration preserving SEO and order history"
 *   badges={[
 *     { label: 'Foundation', color: 'brand', pill: true },
 ...
 *
 * @example
 * // Custom body with metrics (no link)
 * <EntityCard
 *   density="comfortable"
 *   title="Project Acme"
 ...
 *   body={
 *     <Inline gap={4} wrap>
 *       <MetricTile label="MRR" value="1 200" tone="success" />
