...
[![Version](https://img.shields.io/badge/npm-v0.7.5-0ea5e9)](https://github.com/BleizLabs/bleizlabs-ui/pkgs/npm/ui)
[![License](https://img.shields.io/badge/license-MIT-green)](LICENSE)
[![React](https://img.shields.io/badge/React-19-61dafb)](https://react.dev)
[![Next.js](https://img.shields.io/badge/Next.js-16.2-black)](https://nextjs.org)
[![TypeScript](https://img.shields.io/badge/TypeScript-5.6-3178c6)](https://www.typescriptlang.org)

**Current version: [`@bleizlabs/ui@0.7.5`](https://github.com/BleizLabs/bleizlabs-ui/pkgs/npm/ui)** — hygiene sweep: D11 base-class compliance for 3 post-Phase-10 components (`SiteHeader.wrapper` / `SectionHeader.header` / `ZoneCard` add `.root`), `mx.touch-target` mixin now references the v0.7.4 `--size-touch-min` token (consumer override now propagates to BOTH the 6 v0.7.4-migrated sites AND the ~10 components consuming the mixin), Chip interactive variant gains `mx.touch-target` enforcement per D13 / WCAG 2.2 AAA, 6 dedicated component playground routes added (IconButton, Eyebrow, Chip, FileChip, PageHeader, RevealStack — total `/components/*` routes 62 → 68), `COMPONENT_REGISTRY.md` ZoneCard backfill, `docs/component-inventory.md` PageHeader entry corrected (was stale in "per projekt" table since v0.5.0 promotion). Patch-level — additive only, byte-equivalent visual output, zero breaking changes. Library ships **97 components across 10 categories**. Predecessor `v0.7.4` introduced the `--size-touch-min` semantic token; predecessor `v0.7.3` shipped `ZoneCard` (CP9) — Phase 8 Card-presets family at 10/10. See [CHANGELOG.md](CHANGELOG.md) for the full release history.
...
- **Shipped — v0.7.3:** `ZoneCard` preset (Phase 8 CP9) — universal info-card preset (semantic `<section>` landmark, density × tone variations, 5-axis variation envelope) for zoned section-based layouts. Card presets reach 10/10 — Phase 8 family complete.
...
- **Next — v0.7.5 / v0.8.0:** `FilterBar` molecule (universal compose-first filter row with searchable text input + chip toggles + sort/density toggle group; promoted from panel_v2 production usage). Future minor candidates: `Form` orchestrator (controlled multi-input wrapper with validation slot), Chart primitives (line/area extending `BarChart` patterns), Rich editor surface.
