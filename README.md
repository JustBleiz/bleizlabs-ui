...
**Phase 0 ✓ + Phase 1 Layout ✓ + Phase 2 Typography ✓ + Phase 3 Display ✓ + Phase 4 Simple Interactive ✓ + Phase 4 Production Hardening ✓ + Phase 5 Feedback ✓ + Phase 6 Specialized ✓ + Phase 7 Molecules ✓ + Phase 8 Card Presets ✓ + Phase 9 Demo & Docs ✓ + Phase 10 IN PROGRESS (Dialog ✓ + AlertDialog ✓ + Drawer ✓ + Sheet ✓ + Tooltip ✓ + Popover ✓ + DropdownMenu ✓ + ContextMenu ✓ + HoverCard ✓) + E23 FloatingRoot refactor sprint ✓** (delivered 2026-04-14/15 in Epics E03-E24). **47/47 simple atoms + 6/6 molecules + 5/5 Card presets + 9/22 complex interactive = 67/80 total components** + **5 shared floating primitives in `components/utils/floating/`**.
...
**Next:** Phase 10 remaining 13 Complex Interactive components (NavigationMenu, Tabs, Select, Combobox, Slider, Toast, Calendar, DatePicker, InputOTP, Command, ScrollArea, Carousel, Sidebar) — one per Epic. E19-E22 established the floating-family + accessible menu sub-family. **E23 FloatingRoot refactor sprint** extracted 5 composable primitives to `utils/floating/` and migrated 3 consumers (Popover/DropdownMenu/ContextMenu) zero breaking changes. **E24 HoverCard** is the first new-build consumer of E23 primitives (validate-in-production) and pattern-parent for NavigationMenu hover trigger. Next natural picks: **NavigationMenu (CI10)** — leverages HoverCard hover-trigger pattern + adds menubar role; OR **Tabs (CI11)** — new sub-family with `role="tablist"` + roving tabindex (no positioning concerns); OR **Select (CI12)** — first listbox sub-family, would trigger `useMenuKeyboard` extraction decision when Combobox follows.

## Demo showcase

Run `npm run dev` and open [`http://localhost:3000/demo`](http://localhost:3000/demo) to see all 67 components in one page. The demo has a theme toggle button that swaps `[data-theme]` on `<html>` — inspect both light and dark tokens in place.
